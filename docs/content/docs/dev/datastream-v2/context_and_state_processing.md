...
For most methods in `ProcessFunction`, NonPartitionedContext and PartitionedContext are passed in,
depending on whether this method can derive the current partition.

For example, `processRecord`, `onProcessingTimer`, all have clear partition information, so it will receive a `PartitionedContext` as the argument.
...
enum RedistributionMode {
    ...
    /** States are guaranteed to be identical in different partitions, thus redistribution is not a problem. */
    IDENTICAL
}
```
...
{{< img src="/fig/datastream/one-input-state-access.png" alt="one-input-state-access" >}}
...
{{< img src="/fig/datastream/two-input-state-access.png" alt="two-input-state-access" >}}

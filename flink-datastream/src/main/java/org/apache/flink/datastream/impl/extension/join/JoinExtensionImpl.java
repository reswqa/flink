...
/** An internal implementation class for {@link JoinExtension}. */
public class JoinExtensionImpl {
    ...
    public static <IN1, IN2, OUT, W extends Window>
            TwoInputWindowProcessFunction<Iterable<IN1>, Iterable<IN2>, OUT, W> wrapJoinFunction(
                    JoinFunction<IN1, IN2, OUT> joinFunction, JoinExtension.JoinType joinType) {
        switch (joinType) {
            case INNER:
                return new JoinedWindowProcessFunction<>(joinFunction);
            case LEFT_OUTER:
                return new OutJoinedWindowProcessFunction.LeftOuterJoinedWindowProcessFunction<>(
                        joinFunction);
            case RIGHT_OUTER:
                return new OutJoinedWindowProcessFunction.RightOuterJoinedWindowProcessFunction<>(
                        joinFunction);
            case FULL_OUTER:
                return new OutJoinedWindowProcessFunction.FullOuterJoinedWindowProcessFunction<>(
                        joinFunction);
            default:
                throw new UnsupportedOperationException("Unsupported join type : " + joinType);
        }

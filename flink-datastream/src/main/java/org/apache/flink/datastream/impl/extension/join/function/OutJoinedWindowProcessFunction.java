...
/**
 * A special {@link TwoInputWindowProcessFunction} is used to wrap {@link JoinFunction} for
 * performing outer joins within a window.
 */
public abstract class OutJoinedWindowProcessFunction<IN1, IN2, OUT, W extends Window>
    ...
    protected void joinRecordByJoinFunction(
    ...
    public static class LeftOuterJoinedWindowProcessFunction<IN1, IN2, OUT, W extends Window>
        ...
        protected void join(
            ...
            if (!right.iterator().hasNext()) {
                for (IN1 record : left) {
                    joinRecordByJoinFunction(record, null, output, ctx);
                }
            } else {
                for (IN1 leftRecord : left) {
                    for (IN2 rightRecord : right) {
                        joinRecordByJoinFunction(leftRecord, rightRecord, output, ctx);
                    }
                }
            }
        }
        ...
        protected void join(
            ...
            if (!left.iterator().hasNext()) {
                for (IN2 record : right) {
                    joinRecordByJoinFunction(null, record, output, ctx);
                }
            } else {
                for (IN1 leftRecord : left) {
                    for (IN2 rightRecord : right) {

...
public class WindowExtensionImpl {
    ...
    public static <IN1, IN2, OUT, W extends Window>
        ...
        return new InternalTwoInputWindowFunction<>(processFunction, windowAssigner, trigger);
    }
    ...
    public static class TimeWindowsImpl {
        public static WindowAssigner<?, ?> ofSliding(
                Duration windowSize,
                Duration slide,
                WindowExtension.TimeWindows.TimeType timeType) {
            if (timeType == WindowExtension.TimeWindows.TimeType.PROCESSING) {
                return SlidingProcessingTimeWindowsAssigner.of(windowSize, slide);
            } else if (timeType == WindowExtension.TimeWindows.TimeType.EVENT) {
                return SlidingEventTimeWindowsAssigner.of(windowSize, slide);
            } else {

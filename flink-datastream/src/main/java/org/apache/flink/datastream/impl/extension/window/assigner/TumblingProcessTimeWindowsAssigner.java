...
import static org.apache.flink.datastream.impl.extension.window.window.TimeWindowImpl.getWindowStartWithOffset;

/**
 * A special {@link WindowAssigner} for tumbling processing time {@link TimeWindow}, ensuring that
 * each element belongs to only one window.
 */
public class TumblingProcessTimeWindowsAssigner extends WindowAssigner<Object, TimeWindowImpl> {
    ...
    private TumblingProcessTimeWindowsAssigner(
        ...
        if (Math.abs(offset) >= windowSize) {
            throw new IllegalArgumentException(
                    "TumblingProcessingTimeWindows parameters must satisfy abs(offset) < windowSize");
        }
    ...
    public Collection<TimeWindowImpl> assignWindows(
        ...
        if (staggerOffset == null) {
            staggerOffset =
                    windowStagger.getStaggerOffset(context.getCurrentProcessingTime(), windowSize);
        }
        long start =
                getWindowStartWithOffset(
                        now, (globalOffset + staggerOffset) % windowSize, windowSize);
        return Collections.singletonList(new TimeWindowImpl(start, start + windowSize, false));
    }
    ...
    public String toString() {
        return "TumblingProcessTimeWindowsAssigner(" + windowSize + ")";
    }
     ...
     * elements to time windows based on the processing timestamp.
     *
     * @param windowSize The size of the generated windows.
     * @return The time policy.
     */
    public static TumblingProcessTimeWindowsAssigner of(Duration windowSize) {
     ...
     * @return The time policy.
     */
    public static TumblingProcessTimeWindowsAssigner of(Duration windowSize, Duration offset) {
        return new TumblingProcessTimeWindowsAssigner(
    ...
    /**
     * Creates a new {@code TumblingProcessingTimeWindows} {@link WindowAssigner} that assigns
     * elements to time windows based on the processing timestamp, offset and a staggering offset,

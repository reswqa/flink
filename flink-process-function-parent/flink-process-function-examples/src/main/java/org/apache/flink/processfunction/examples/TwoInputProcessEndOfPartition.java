...
public class TwoInputProcessEndOfPartition {
    ...
    private static void keyedAndNonKeyed(boolean sortInputs) throws Exception {
        ExecutionEnvironment env = ExecutionEnvironment.getExecutionEnvironment();
        Configuration configuration = new Configuration();
        configuration.setString(
                "execution.sorted-inputs.enabled",
        ...
        env.tmpWithConfiguration(configuration);
        env.tmpSetRuntimeMode(RuntimeExecutionMode.BATCH);
        KeyedPartitionStream<Integer, Integer> keyedSource =
                env.fromSource(
                                Sources.collection(Arrays.asList(1, 2, 3, 4, 5, 6)),
                                WatermarkStrategy.noWatermarks(),
                                "number-source")
                        .keyBy(record -> record % 3);
        NonKeyedPartitionStream.ProcessConfigurableAndNonKeyedPartitionStream<String>
                nonKeyedSource =
                        env.fromSource(
                                Sources.collection(Arrays.asList("1", "2", "3", "4", "5", "6")),
                                WatermarkStrategy.noWatermarks(),

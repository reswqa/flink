...
abstract class MutableHashTableTestBase {
    ...
    public final TypePairComparator<IntPair, IntList> pairComparatorPL =
            new IntPairListPairComparator();
    ...
    private final TypePairComparator<StringPair, StringPair> pairComparatorS =
            new StringPairPairComparator();

    protected abstract <T> AbstractMutableHashTable<T> getHashTable(
    ...
    void testDifferentProbers() {
        ...
        AbstractHashTableProber<IntPair, IntPair> prober1 =
    ...
    void testMultipleProbers() throws Exception {
        ...
        AbstractHashTableProber<IntList, IntList> listProber =
                table.getProber(comparatorV, pairComparatorV);

        AbstractHashTableProber<IntPair, IntList> pairProber =
                table.getProber(intPairComparator, pairComparatorPL);
    ...
    void testVariableLengthBuildAndRetrieveMinorityUpdated() throws Exception {
        ...
        for (int i = 0; i < NUM_LISTS; i += STEP_SIZE) {
            overwriteLists[i / STEP_SIZE].setKey(
                    overwriteLists[i / STEP_SIZE].getKey() * STEP_SIZE);
    ...
    void testVariableLengthStringBuildAndRetrieve() throws IOException {
        ...
        MutableObjectIterator<StringPair> buildInput =
                new UniformStringPairGenerator(NUM_PAIRS, 1, false);

        MutableObjectIterator<StringPair> probeTester =
                new UniformStringPairGenerator(NUM_PAIRS, 1, false);

        MutableObjectIterator<StringPair> updater =
                new UniformStringPairGenerator(NUM_PAIRS, 1, false);

        MutableObjectIterator<StringPair> updateTester =
                new UniformStringPairGenerator(NUM_PAIRS, 1, false);
        ...
        AbstractHashTableProber<StringPair, StringPair> prober =

...
/**
 * Implementation of the {@link TypeComparator} interface for the pact record. Instances of this
 * class are parameterized with which fields are relevant to the comparison.
 */
public final class RecordComparator extends TypeComparator<Record> {
     ...
     */
    private static final int[] HASH_SALT =
            new int[] {
    ...
    /**
     * Creates a new comparator that compares Pact Records by the subset of fields as described by
     * the given key positions and types. All order comparisons will assume ascending order on all
     * fields.
     *
    ...
    /**
     * Creates a new comparator that compares Pact Records by the subset of fields as described by
     * the given key positions and types.
     *
     * @param keyFields The positions of the key fields.
     * @param keyTypes The types (classes) of the key fields.
     * @param sortDirection The direction for sorting. A value of <i>true</i> indicates ascending
     *     for an attribute, a value of <i>false</i> indicated descending. If the parameter is
     *     <i>null</i>, then all order comparisons will assume ascending order on all fields.
     */
    ...
    private RecordComparator(RecordComparator toCopy) {
        ...
        try {
            ...
            // this should never happen, because the classes have been instantiated before. Report
            // for debugging.
            throw new RuntimeException(
                    "Could not instantiate key classes when duplicating RecordComparator.", ex);
        }
    ...
    public int compare(Record first, Record second) {
        ...
        try {
            for (; i < this.keyFields.length; i++) {
                ...
                Comparable k2 =
                        (Comparable)
                                second.getField(this.keyFields[i], this.transientKeyHolders[i]);
    ...
    public int compareSerialized(DataInputView source1, DataInputView source2) throws IOException {
        ...
        for (int i = 0; i < this.keyFields.length; i++) {
            @SuppressWarnings("rawtypes")
            final Comparable k1 =
                    (Comparable) this.temp1.getField(this.keyFields[i], this.keyHolders[i]);
            @SuppressWarnings("rawtypes")
            final Comparable k2 =
                    (Comparable)
                            this.temp2.getField(this.keyFields[i], this.transientKeyHolders[i]);
    ...
    public void putNormalizedKey(Record record, MemorySegment target, int offset, int numBytes) {
        ...
        try {
            for (; i < this.numLeadingNormalizableKeys & numBytes > 0; i++) {
                ...
                ((NormalizableKey<?>)
                                record.getField(this.keyFields[i], this.transientKeyHolders[i]))
    ...
    public int extractKeys(Object record, Object[] target, int index) {
        throw new UnsupportedOperationException(
                "Record does not support extactKeys and "

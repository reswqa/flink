...
class ForStSyncAggregatingState<K, N, T, ACC, R>
     ...
     */
    private ForStSyncAggregatingState(
    ...
    ForStSyncAggregatingState<K, N, T, ACC, R> setAggFunction(
            AggregateFunction<T, ACC, R> aggFunction) {
        this.aggFunction = aggFunction;
        return this;
    }
    ...
    static <K, N, SV, S extends State, IS extends S> IS update(
            ...
            IS existingState) {
        return (IS)
                ((ForStSyncAggregatingState<K, N, ?, SV, ?>) existingState)
                        .setAggFunction(

 ...
 */
class ForStSyncMapState<K, N, UK, UV> extends AbstractForStSyncState<K, N, Map<UK, UV>>
        implements InternalMapState<K, N, UK, UV> {

    private static final Logger LOG = LoggerFactory.getLogger(ForStSyncMapState.class);
    ...
    public void clear() {
        try (ForStIteratorWrapper iterator =
                        ForStOperationUtils.getForStIterator(
                                backend.db, columnFamily, backend.getReadOptions());
                ForStDBWriteBatchWrapper rocksDBWriteBatchWrapper =
                        new ForStDBWriteBatchWrapper(
                                backend.db,
    ...
    @Override
    protected ForStSyncMapState<K, N, UK, UV> setValueSerializer(
            TypeSerializer<Map<UK, UV>> valueSerializer) {
    ...
    static <UK, UV, K, N, SV, S extends State, IS extends S> IS update(
            ...
            IS existingState) {
        return (IS)
                ((ForStSyncMapState<K, N, UK, UV>) existingState)

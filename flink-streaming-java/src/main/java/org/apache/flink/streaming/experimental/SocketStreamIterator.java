 ...
 *
 * <p>The iterator's constructor opens a server socket. In the first call to {@link #next()} or
 * {@link #hasNext()}, the iterator waits for a socket to connect, and starts receiving,
...
public class SocketStreamIterator<T> implements Iterator<T> {
     ...
     * @throws IOException thrown if socket cannot be opened
     */
    public SocketStreamIterator(int port, InetAddress address, TypeSerializer<T> serializer)
            throws IOException {

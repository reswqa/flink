...
import org.apache.flink.api.common.typeinfo.utils.{TypeSerializer, TypeSerializerSchemaCompatibility, TypeSerializerSnapshot}
  ...
  override def resolveSchemaCompatibility(oldSerializerSnapshot: TypeSerializerSnapshot[E#Value])
    ...
    for ((oldEnumName, index) <- oldEnumSerializerSnapshot.enumConstants) {
      try {
        ...
        if (enumName != oldEnumName) return TypeSerializerSchemaCompatibility.incompatible()
      } catch {

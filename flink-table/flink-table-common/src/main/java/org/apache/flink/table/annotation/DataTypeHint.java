 ...
 * function parameters and return types, structured classes, or fields of structured classes. An
 * implementer can choose to what extent the default extraction logic should be modified.
 *
 * <p>The following examples show how to explicitly specify data types, how to parameterize the
 * extraction logic, or how to accept any data type as an input data type:
 *
 * <p>{@code @DataTypeHint("INT")} defines an INT data type with a default conversion class.
 *
 * <p>{@code @DataTypeHint(value = "TIMESTAMP(3)", bridgedTo = java.sql.Timestamp.class)} defines a
 ...
 *
 * <p>{@code @DataTypeHint(version = V1, allowRawGlobally = TRUE)} parameterizes the extraction by
 ...
 *
 * <p>{@code @DataTypeHint(bridgedTo = MyPojo.class, allowRawGlobally = TRUE)} defines that a type
 ...
 * data type.
 *
 * <p>Note: All hint parameters are optional. Hint parameters defined on top of a structured type
 * are inherited by all (deeply) nested fields unless annotated differently. For example, all
 * occurrences of {@link java.math.BigDecimal} will be extracted as {@code DECIMAL(12, 2)} if the
 * enclosing structured class is annotated with {@code @DataTypeHint(defaultDecimalPrecision = 12,
 * defaultDecimalScale = 2)}. Individual field annotations allow to deviate from those default
 * values.
 *
 * <p>A data type hint on top of a table or aggregate function is similar to defining {@link
 * FunctionHint#output()} for the output type of the function.
 *
 * @see FunctionHint
 */
@PublicEvolving
...
public @interface DataTypeHint {
    ...
    /**
     * The explicit string representation of a data type. See {@link DataTypes} for a list of
     * supported data types. For example, {@code INT} for an integer data type or {@code DECIMAL(12,
     * 5)} for decimal data type with precision 12 and scale 5.
     *
     * <p>Use an unparameterized {@code RAW} string for explicitly declaring an opaque data type
     * without entering a full type string. For Flink's default RAW serializer, use
     * {@code @DataTypeHint("RAW")} or more specific {@code @DataTypeHint(value = "RAW", bridgedTo =
     * MyCustomClass.class)}. For a custom RAW serializer, use {@code @DataTypeHint(value = "RAW",
     ...
     * the table ecosystem.
     *
     * <p>If an explicit data type has been defined via {@link #value()}, a supported conversion
     ...
     * define the class prefix {@code "org.joda.time"}. Some classes might be handled as structured
     ...
     *
     * <p>For example, one can define the class prefix {@code "org.joda.time",
     * "java.math.BigDecimal"} which means that all Joda time classes and Java's {@link
     * java.math.BigDecimal} will be handled as RAW data types regardless if they could be mapped to
     ...
     *
     * <p>If RAW types cannot be avoided, they should be enabled only in designated areas (i.e.,
     * within package prefixes) in order to not swallow all errors. However, compared to {@link
     * #allowRawPattern()}, this parameter forces to skip the extraction entirely for the given
     ...
     *
     * <p>By default, decimals are not extracted from classes such as {@link java.math.BigDecimal}
    ...
    /**
     * Defines a default year precision for all year-month intervals that are extracted. If set to
     * {@code 0}, an {@code INTERVAL MONTH} data type is extracted.
     *
     * <p>By default, {@code INTERVAL YEAR(4) TO MONTH} data types are extracted from classes such
     * as {@link java.time.Period}.
     */

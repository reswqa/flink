...
final class DataTypeTemplate {
    ...
    /**
     * Creates an instance from the given {@link DataTypeHint}. Resolves an explicitly defined data
     * type if {@link DataTypeHint#value()} and/or {@link DataTypeHint#bridgedTo()} are defined.
     */
    static DataTypeTemplate fromAnnotation(DataTypeFactory typeFactory, DataTypeHint hint) {
        ...
        if (typeName != null || conversionClass != null) {
            ...
            // a template can contain a data type but in order to extract a data type we might need
            // a template

...
public class RowDataCustomPartitionerWrapper extends StreamPartitioner<RowData> {
    private final RowDataCustomPartitioner partitioner;

    public RowDataCustomPartitionerWrapper(RowDataCustomPartitioner partitioner) {
        this.partitioner = partitioner;
    }

    @Override
    public int selectChannel(SerializationDelegate<StreamRecord<RowData>> record) {
        int partition = partitioner.partition(record.getInstance().getValue(), numberOfChannels);

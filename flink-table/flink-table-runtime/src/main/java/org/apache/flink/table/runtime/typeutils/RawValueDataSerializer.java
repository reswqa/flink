...
public final class RawValueDataSerializer<T> extends TypeSerializer<RawValueData<T>> {
    ...
    public RawValueData<T> copy(RawValueData<T> from) {
        ...
        byte[] bytes =
                BinarySegmentUtils.copyToBytes(
                        rawValue.getSegments(), rawValue.getOffset(), rawValue.getSizeInBytes());
        T newJavaObject =
    ...
    public void serialize(RawValueData<T> record, DataOutputView target) throws IOException {
        ...
        BinarySegmentUtils.copyToView(
                rawValue.getSegments(), rawValue.getOffset(), rawValue.getSizeInBytes(), target);
    }
    ...
    @Override
    public RawValueData<T> deserialize(RawValueData<T> record, DataInputView source)
            throws IOException {
    ...
    public static final class RawValueDataSerializerSnapshot<T>
        ...
        protected TypeSerializer<?>[] getNestedSerializers(
                RawValueDataSerializer<T> outerSerializer) {
            return new TypeSerializer[] {outerSerializer.serializer};
        }

        @Override
        @SuppressWarnings("unchecked")
        protected RawValueDataSerializer<T> createOuterSerializerWithNestedSerializers(

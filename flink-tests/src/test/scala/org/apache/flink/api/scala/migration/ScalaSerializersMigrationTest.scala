...
class ScalaSerializersMigrationTest {
  ...
  def testStableAnonymousClassnameGeneration(): Unit = {
    ...
    Assert.assertEquals(
      ...
      caseClassInfo.createSerializer(new SerializerConfig).getClass
    )
    ...
    Assert.assertEquals(
      classOf[ScalaCaseClassSerializer[CustomCaseClass]],
      caseClassWithNestingInfo.createSerializer(new SerializerConfig).getClass
    )
    assertPreviouslyGeneratedClassExists(
    ...
    Assert.assertEquals(
      classOf[ScalaCaseClassSerializer[CustomCaseClass]],
      tryInfo.elemTypeInfo.createSerializer(new SerializerConfig).getClass
    )
    ...
    Assert.assertEquals(
      classOf[ScalaCaseClassSerializer[CustomCaseClass]],
      optionInfo.getElemTypeInfo.createSerializer(new SerializerConfig).getClass
    )
    ...
    Assert.assertEquals(
      classOf[ScalaCaseClassSerializer[CustomCaseClass]],
      eitherInfo.leftTypeInfo.createSerializer(new SerializerConfig).getClass
    )
  }
